...
// tskr/ui control kit — Select. Button trigger + custom listbox popover (the
// open menu native <select> won't style). Keyboard: Up/Down/Home/End move the
// active option, Enter/Space commit, Esc closes (usePopover).
import "./controls.css";
...
<template>
  <div class="tk-field tk-selectfield">
    ...
    <div class="tk-select">
      ...
      >
        ...
        <ChevronDown class="tk-select__chev" :size="15" :stroke-width="2" />
      </button>
      <ul
        v-show="open"
        ref="panel"
      ...
      >
        ...
        >
          <span>{{ o.label }}</span>
          <Check v-if="o.value === value" class="tk-select__tick" :size="14" :stroke-width="2.5" />
        </li>
      </ul>
    </div>
  </div>
