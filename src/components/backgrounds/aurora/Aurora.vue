...
const props = withDefaults(defineProps<AuroraProps>(), {
  ...
  auroraColor1: "#00aaff",
  auroraColor2: "#ff00aa",
  speed: 1.0, // 1.0 = normal speed, higher for faster animation
  intensity: 3.0, // 1.0 = normal intensity, higher for more vibrant colors
  densityX: 2.5, // 1.0 = normal density, higher for more detail
  densityY: 0.0, // 1.0 = normal density, higher for more detail
  verticalFalloff: 1.0,
  opacity: 1.0,
  noiseAmplitude: 0.3,
...
const init = (): void => {
  ...
  try {
    ...
    auroraUniforms = {
      ...
      uDensityY: { value: props.densityY },
      uVerticalFalloff: { value: props.verticalFalloff },
...
const onResize = (): void => {
  ...
  renderer.setPixelRatio(
    Math.min(window.devicePixelRatio * props.resolution, 2),
  ); // Cap pixel ratio
...
watch(
  () => props.densityX,
  (newVal) => {
    if (auroraUniforms) auroraUniforms.uDensityX.value = newVal;
  },
);
...
watch(
  () => props.densityY,
  (newVal) => {
    if (auroraUniforms) auroraUniforms.uDensityY.value = newVal;
  },
);

watch(
  () => props.verticalFalloff,
  (newVal) => {
    if (auroraUniforms) auroraUniforms.uVerticalFalloff.value = newVal;
  },
);
...
watch(
  () => props.opacity,
  (newVal) => {
    if (auroraUniforms) auroraUniforms.uOpacity.value = newVal;
  },
);
...
watch(
  ...
  (newVal) => {
    if (auroraUniforms) auroraUniforms.uNoiseAmplitude.value = newVal;
  },
);
...
<template>
  <div ref="containerRef" class="absolute left-0 top-0 w-full h-full"></div>
</template>
